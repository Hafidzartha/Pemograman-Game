...
Engine::Trex_MainMenuScreen::Trex_MainMenuScreen()
{
	text = NULL;
}

void Engine::Trex_MainMenuScreen::Init()
{
	// Create a Texture
	...
	// Create Text
	text = (new Text("8-bit Arcade In.ttf", 100, game->GetDefaultTextShader()))->SetText("Trex AJA");
	text->SetPosition((game->GetSettings()->screenWidth * 0.5f) - (text->GetTextWidth() * 0.5f), game->GetSettings()->screenHeight - 200.0f)
		->SetColor(235, 229, 52);
	...
	Texture* backsround = new Texture("MainScreenBg.jpg");
	backgroundSprite = new Sprite(backsround, game->GetDefaultSpriteShader(), game->GetDefaultQuad());
	backgroundSprite->SetPosition(0, 0)->SetScale(0.3f);

}


void Engine::Trex_MainMenuScreen::Update()
{
...
void Engine::Trex_MainMenuScreen::Draw()
{
	// Render all buttons
